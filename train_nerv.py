...
from nerv_model import VideoDataSet, HNeRV, HNeRVDecoder, TransformInput
from nerv_utils import *
...
def get_codebook_entry(codebook, indices, use_l2_norm=False):
    embedding = torch.nn.Embedding.from_pretrained(codebook, freeze=True)
    if len(indices.shape) == 1:
        z_quantized = embedding(indices)
    elif len(indices.shape) == 2:
        z_quantized = torch.einsum('bd,dn->bn', indices, embedding.weight)
    else:
